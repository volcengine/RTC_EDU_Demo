...
public class RoomPageLayoutManager extends RecyclerView.LayoutManager implements
    ...
    private void recycleAndFillItems(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        PagerConfig.error("child count = " + getChildCount());
        startPos = pageIndex * mOnePageSize;
        stopPos = startPos + mOnePageSize - 1;
        if (stopPos >= getItemCount()) {
            stopPos = getItemCount() - 1;
        }
        PagerConfig.error("visible from " + startPos + " to " + stopPos);
        if (mPageListener != null) {
    ...
    private int getPageIndexByOffset() {
        ...
        PagerConfig.info("getPageIndexByOffset pageIndex = " + pageIndex);
        return pageIndex;
    }
    ...
    int findNextPageFirstPos() {
        ...
        PagerConfig.error("computeScrollVectorForPosition next = " + page);
    ...
    public void scrollToPage(int pageIndex) {
        ...
        PagerConfig.error("mTargetOffsetXBy = " + mTargetOffsetXBy);
        PagerConfig.error("mTargetOffsetYBy = " + mTargetOffsetYBy);
