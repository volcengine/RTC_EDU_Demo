...
import { tosConfig } from '@src/config';
...
import { setBoardPagePreviewOpen } from '@src/store/slices/ui';
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const transcodeRes = await BoardClient.room
        ?.createTranscodeTask({
          ...
          onProgress: (transcodePercent: number, transcodeStatus: TaskStatus, taskId: string) => {
            ...
            //   setTranscodingStatus(transcodeStatus);
            //   setTaskId(taskId);
          },
        })
        .catch((err: Error) => {
      ...
      if (transcodeRes) {
        ...
        BoardClient.room?.createWhiteBoard(id % 10000000, pageConfig, {},  (file.name.split('.')[0] || '').slice(0, 9));
      }
    } catch (err) {
      console.error(err);
      Message.error('上传失败');
