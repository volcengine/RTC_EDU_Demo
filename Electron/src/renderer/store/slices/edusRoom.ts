...
import {
  ...
  IEdusOnStartShare,
} from '@/renderer/pages/Edus/hooks/informType';
...
export const roomSlice = createSlice({
  ...
  initialState,
  reducers: {
    setEdusRoom: (state, action: PayloadAction<EdusRoomState>) => {
    ...
    stopShare: (state, action: PayloadAction<{}>) => {
    ...
    remoteUserChangeMic: (
      state,
      action: PayloadAction<Pick<IEdusOnOperateSelfMic, 'user_id' | 'operate'>>
    ) => {
      state.remoteUsers?.forEach((user) => {
        if (user.user_id === action.payload.user_id) {
