...
import {
  ...
  IOnStartShare,
} from '@src/pages/View/hooks/informType';
import {
  ApplyType,
  BaseUser,
  DeviceState,
...
export interface MeetingRoomState {
  ...
  localUser: Partial<IMeetingUser>;
...
export const roomSlice = createSlice({
  ...
  initialState,
  reducers: {
    setMeetingRoom: (state, action: PayloadAction<MeetingRoomState>) => {
    ...
    remoteUserChangeMic: (
      state,
      action: PayloadAction<Pick<IOnOperateSelfMic, 'user_id' | 'operate'>>
    ) => {
      state.remoteUsers?.forEach((user) => {
        if (user.user_id === action.payload.user_id) {
