...
import {
  ...
  IEdubStartShare,
  IEdubStopShare,
} from '@/scene/Edub/hooks/informType';
import {
  DeviceState,
  BaseUser,
  Permission,
...
import { ILinkmicApplyList, JoinEdubRoomRes } from '@/scene/Edub/apis/rtsApi';
...
export const edubRoomSlice = createSlice({
  ...
  reducers: {
    ...
    stopShare: (state, action: PayloadAction<IEdubStopShare>) => {
    ...
    setAudioInfo: (
      ...
      if (isLocal) {
        ...
        // 本端如果是老师
        if (state.localUser.user_role === UserRole.Host) {
          state.teacher!.audioPropertiesInfo = audioInfo;
        }
        // 本端不是老师
        state.linkmic_user_list = state.linkmic_user_list?.map((user) => {
