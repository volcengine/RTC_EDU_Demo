...
import {
  ...
  IEdusOnJoinRoom,
  IEdusOnLeaveRoom,
  IEdusOnOperateAllMic,
  IEdusOnOperateSelfCamera,
  IEdusOnOperateSelfMic,
  IEdusOnOperateSelfMicApply,
  IEdusOnSharePermissionApply,
  IEdusOnStartShare,
} from '@/scene/Edus/hooks/informType';
import {
  DeviceState,
  BaseUser,
  Permission,
...
export interface IEdusUser extends BaseUser {
  isActive?: boolean;
}

export interface EdusRoomState {
  app_id?: string;
  room_id?: string;
  room_name?: string;
  host_user_id?: string;
  ...
  localUser: Partial<IEdusUser & { isLocal: boolean; is_silence?: Silence }>;
  remoteUsers: (IEdusUser & {
    rtcStatus: {
      joined?: boolean;
      screen?: boolean;
      stream?: boolean;
...
const initialState: EdusRoomState = {
 ...
 * 房间信息
 */
export const edusRoomSlice = createSlice({
  name: 'edusRoom',
  initialState,
  reducers: {
    /**
    ...
    remoteUserJoinRoom: (state, action: PayloadAction<IEdusOnJoinRoom>) => {
    ...
    remoteUserLeaveRoom: (state, action: PayloadAction<IEdusOnLeaveRoom>) => {
      const findIndex = state.remoteUsers!.findIndex(
        (user) => user.user_id === action.payload.user.user_id
      );
    ...
    stopShare: (state, action: PayloadAction<IEdusOnFinishShare>) => {
    ...
    startShare: (state, action: PayloadAction<IEdusOnStartShare>) => {
      state.remoteUsers?.forEach((user) => {
    ...
    muteAll: (state, action: PayloadAction<IEdusOnOperateAllMic>) => {
    ...
    remoteUserChangeMic: (state, action: PayloadAction<IEdusOnOperateSelfMic>) => {
      state.remoteUsers?.forEach((user) => {
        if (user.user_id === action.payload.user_id) {
    ...
    remoteUserChangeCamera: (state, action: PayloadAction<IEdusOnOperateSelfCamera>) => {
      state.remoteUsers?.forEach((user) => {
        if (user.user_id === action.payload.user_id) {
    ...
    remoteApply: (
      state,
      action: PayloadAction<
        (IEdusOnOperateSelfMicApply | IEdusOnSharePermissionApply) & {
          applying: ApplyType;
        }
      >
    ) => {
...
export const {
  ...
  setActiveSpeaker,
  setRtcStatus,
} = edusRoomSlice.actions;

export default edusRoomSlice.reducer;
