...
DeviceSetting::DeviceSetting(QWidget* parent) : QWidget(parent) {
	...
	connect(&EduRTCEngineWrap::instance(),
		...
		[=](std::vector<AudioVolumeInfoWrap> volumes) {
			...
			ui.vol_mic->setValue(volumes[0].volume);
		});

	connect(&EduRTCEngineWrap::instance(),
		&EduRTCEngineWrap::sigOnAudioPlaybackDeviceTestVolume,
		this,
		[=](int volume) {
			ui.vol_play->setValue(volume);
		});
...
void DeviceSetting::startTest() {
  ...
  EduRTCEngineWrap::setAudioVolumeIndicate(200);
