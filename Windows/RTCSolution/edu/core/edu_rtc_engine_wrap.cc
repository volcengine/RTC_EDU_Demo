...
int EduRTCEngineWrap::init() {
	...
	connect(&RtcEngineWrap::instance(), &RtcEngineWrap::sigOnRoomStateChanged,
		&instance(), [=](std::string room_id, std::string uid, int state, std::string extra_info) {
			emit instance().sigOnRoomStateChanged(room_id, uid, state, extra_info);
			if (state == bytertc::kErrorCodeDuplicateLogin) {
				//相同账号进房，强制退出
				emit instance().sigForceExitRoom();
				emit SceneSelectWidget::instance().sigLogOut();
				vrd::util::showToastInfo(("用户在另一台设备加入"));
			}
		});

	QObject::connect(&RtcEngineWrap::instance(), &RtcEngineWrap::sigOnServerMessageSendResult,
		&instance(), [=](int64_t msgid, int error, const bytertc::ServerACKMsg& msg) {
			if (error == bytertc::UserMessageSendResult::kUserMessageSendResultNotLogin) {
				//相同账号进房，强制退出
				emit instance().sigForceExitRoom();
				emit SceneSelectWidget::instance().sigLogOut();
				vrd::util::showToastInfo(("用户在另一台设备加入"));
	...
	connect(&RtcEngineWrap::instance(), &RtcEngineWrap::sigOnLocalAudioVolumeIndication, &instance(),
		[](std::vector<AudioVolumeInfoWrap> volumes) {
			emit instance().sigAudioVolumeIndication(volumes);
		});

	connect(&RtcEngineWrap::instance(), &RtcEngineWrap::sigOnAudioPlaybackDeviceTestVolume, &instance(),
		[](int volume) {
			emit instance().sigOnAudioPlaybackDeviceTestVolume(volume);
		});
...
void EduRTCEngineWrap::createMainRoom(const std::string& room) {
	...
	RtcEngineWrap::instance().setMainRoomId(room);
}
...
void EduRTCEngineWrap::publishGroupRoom(bytertc::MediaStreamType type) {
	instance().group_room_->publishStream(type);
}

void EduRTCEngineWrap::unPublishGroupRoom(bytertc::MediaStreamType type) {
	instance().group_room_->unpublishStream(type);
}
