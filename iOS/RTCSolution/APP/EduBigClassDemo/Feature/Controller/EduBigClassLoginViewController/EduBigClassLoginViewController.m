...
@property (nonatomic, strong) EduBCUnderlinedButtonBar* buttonBar;
@property (nonatomic, strong) BaseButton *createEduBigClassBtn;
@property (nonatomic, strong) UIView *infoView;
@property (nonatomic, weak) EduBCFeedbackView *EduBCFeedbackView;
...
@implementation EduBigClassLoginViewController
...
- (void)viewDidLoad {
    ...
    NSString *sdkVer = [EduBigClassRTCManager getSdkVersion];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [[EduBigClassRTCManager shareRtc] startPreview:self.videoView];
}
...
- (void)onClickEnterRoom:(UIButton *)sender {
    ...
    __block EduBCRoomVideoSession *eduBCRoomVideoSession = [[EduBCRoomVideoSession alloc] initWithUid:[LocalUserComponent userModel].uid];
    eduBCRoomVideoSession.name = self.userIdTextField.text;
    eduBCRoomVideoSession.appid = self.currentAppid;
    eduBCRoomVideoSession.roomId = self.roomIdTextField.text;
    eduBCRoomVideoSession.isLoginUser = YES;
    eduBCRoomVideoSession.isEnableAudio = self.enableAudioBtn.status == ButtonStatusNone;
    eduBCRoomVideoSession.isEnableVideo = self.enableVideoBtn.status == ButtonStatusNone;
    eduBCRoomVideoSession.isHost = NO;
    eduBCRoomVideoSession.isSharingScreen = NO;
    eduBCRoomVideoSession.isSharingWhiteBoard = NO;

    [PublicParameterComponent share].roomId = eduBCRoomVideoSession.roomId;

    [[ToastComponent shareToastComponent] showLoading];
    __weak __typeof(self) wself = self;
    [EduBigClassRTMManager joinEduBigClass:eduBCRoomVideoSession block:^(int result, EduBigClassControlRoomModel *roomModel, NSArray<EduBCRoomVideoSession *> *userLists) {
        if (result == 200) {
            [wself jumpToRoomVC:eduBCRoomVideoSession roomModel:roomModel userLists:userLists];
        } else {
...
- (void)jumpToRoomVC:(EduBCRoomVideoSession *)localSession roomModel:(EduBigClassControlRoomModel *)roomModel userLists:(NSArray<EduBCRoomVideoSession *> *)userLists {
    EduBCRoomViewController *roomVC = [[EduBCRoomViewController alloc] initWithLocalVideoSession:localSession withRoomModel:roomModel andUsers:userLists];
...
- (void)onClickEnableVideo:(BaseButton *)sender {
    if (sender.status != ButtonStatusIllegal) {
        ...
        [[EduBigClassRTCManager shareRtc] resetPreview:isEnableVideo ? self.videoView : nil];
        [[EduBigClassRTCManager shareRtc] enableLocalVideo:isEnableVideo];
    } else {
        AlertActionModel *alertCancelModel = [[AlertActionModel alloc] init];
        alertCancelModel.title = @"取消";
        AlertActionModel *alertModel = [[AlertActionModel alloc] init];
...
- (void)closeRoomAction:(BOOL)isEnableAudio isEnableVideo:(BOOL)isEnableVideo {
    /*if (!self.EduBCFeedbackView) {
        __block EduBCFeedbackView *EduBCFeedbackView = [[EduBCFeedbackView alloc] init];
        self.EduBCFeedbackView = EduBCFeedbackView;
        [EduBCFeedbackView show];
        EduBCFeedbackView.clickCancelBlock = ^{
            EduBCFeedbackView = nil;
        };
    }*/
...
- (BaseButton *)createEduBigClassBtn {
    if (!_createEduBigClassBtn) {
        _createEduBigClassBtn = [[BaseButton alloc] init];
        [_createEduBigClassBtn setTitle:@"我是学生" forState:UIControlStateNormal];
        [_createEduBigClassBtn sizeToFit];

        [_createEduBigClassBtn addTarget:self action:@selector(onButtonBarClicked:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _createEduBigClassBtn;
}


- (EduBCUnderlinedButtonBar *)buttonBar {
    if (!_buttonBar) {
        CGFloat width = CGRectGetWidth(self.createEduBigClassBtn.frame) + CGRectGetWidth(self.createEduBigClassBtn.frame) + 44;
        _buttonBar = [[EduBCUnderlinedButtonBar alloc] initWithButtons:@[self.createEduBigClassBtn] width:width height:45 underlineColor:[UIColor whiteColor] underlineTrackColor:[UIColor clearColor] underlineHeight:5 underlineMargin:45 backgroundColor:[UIColor clearColor] animated:YES];
    }
    return _buttonBar;
}
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = @"大班课";
    }
    return _titleLabel;
}

- (void)dealloc {
    [[EduBigClassRTCManager shareRtc] disconnect];
    [[NSNotificationCenter defaultCenter] postNotificationName:NotificationJoinOrExit object:nil];
