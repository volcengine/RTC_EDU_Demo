...
#import "EduBCRoomCollectionViewLayout.h"
#import "Masonry.h"
#import "Constants.h"
#import "EduBCRoomViewModel+Sort.h"

@interface EduBCRoomCollectionView()<UICollectionViewDelegate>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) UIImageView *waitView;
@property (nonatomic, strong) UIPageControl *pageControl;
@property (nonatomic, strong) EduBCRoomCollectionViewDataSource *dataSource;
@property (nonatomic, strong) EduBCRoomCollectionViewLayout* collectionViewLayout;
...
@implementation EduBCRoomCollectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self.waitView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.equalTo(self.mas_width).multipliedBy(0.95);
...
- (void)bindVideoSessions:(NSArray<EduBCRoomVideoSession *> *)videoSessions {
    ...
    [self checkFlod];
    self.waitView.hidden = videoSessions.count;
}
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[EduBCRoomCollectionViewCell class] forCellWithReuseIdentifier:NSStringFromClass([EduBCRoomCollectionViewCell class])];
    }
...
- (UIImageView *)waitView {
    if (!_waitView) {
        _waitView = [[UIImageView alloc] initWithImage:[ThemeManager imageNamed:@"meeting_room_wait_t"]];
    }
    return _waitView;
}

- (EduBCRoomCollectionViewDataSource *)dataSource {
    if (!_dataSource) {
        _dataSource = [[EduBCRoomCollectionViewDataSource alloc] init];
    }
    return _dataSource;
}

- (EduBCRoomCollectionViewLayout *)collectionViewLayout {
    if (!_collectionViewLayout) {
        _collectionViewLayout = [[EduBCRoomCollectionViewLayout alloc] init];
