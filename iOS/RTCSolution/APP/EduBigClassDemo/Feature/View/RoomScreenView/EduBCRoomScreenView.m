...
@property (nonatomic, strong) EduBCRoomUserView *screenVideoView;

@end


@implementation EduBCRoomScreenView
...
- (void)setSharingVideoSession:(EduBCRoomVideoSession *)sharingVideoSession {
...
- (void)updateIconPosition:(CGFloat)line {
    [self.scaleBotton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(self).offset(line);
        make.bottom.mas_equalTo(self).offset(FIGMA_SCALE(-32));
        make.size.mas_equalTo(CGSizeMake(FIGMA_SCALE(48), FIGMA_SCALE(48)));
    }];
...
- (EduBCRoomUserView *)screenVideoView {
    if (!_screenVideoView) {
        _screenVideoView = [[EduBCRoomUserView alloc] init];
...
//            EduBCRoomItemButton *button = (EduBCRoomItemButton *)sender;
////            if ([wself.delegate respondsToSelector:@selector(EduBCRoomBottomView:itemButton:didSelectStatus:)]) {
////                [wself.delegate EduBCRoomBottomView:wself itemButton:button didSelectStatus:RoomBottomItemMic];
////            }
//        }];
//    }
    return nil;
