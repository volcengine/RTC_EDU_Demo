...
@implementation EduBCUserListCell
...
- (void)setVideoSession:(EduBCRoomVideoSession *)videoSession {
    ...
    self.EduBCAvatarView.text = videoSession.name;
...
- (void)handBtnAction:(BaseButton *)sender {
    if (self.isShareHand) {
        ...
        alertCancelModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantSharePermission:self.videoSession.uid result:NO];
            self.enableShareBtn.status = ButtonStatusActive;
        ...
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantSharePermission:self.videoSession.uid result:YES];
            self.enableShareBtn.status = ButtonStatusNone;
        ...
        alertCancelModel.title = @"取消";
        alertCancelModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
           //[EduBigClassRTMManager grantMicPermission:self.videoSession.uid result:NO];
            self.enableAudioBtn.status = ButtonStatusActive;
        ...
        alertModel.title = @"同意";
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantMicPermission:self.videoSession.uid result:YES];
            self.enableAudioBtn.status = ButtonStatusNone;
...
- (void)createUIComponent {
    [self.contentView addSubview:self.EduBCAvatarView];
    ...
    [self.userIDLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.EduBCAvatarView.mas_right).mas_offset(9);
    ...
    [self.avatarSpeakView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.EduBCAvatarView);
    }];
}
...
- (EduBCAvatarView *)EduBCAvatarView {
    if (!_EduBCAvatarView) {
        ...
        _EduBCAvatarView.fontSize = 20;
        _EduBCAvatarView.layer.masksToBounds = YES;
        _EduBCAvatarView.layer.cornerRadius = 20;
    }
