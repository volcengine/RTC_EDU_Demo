...
@implementation EduBCPermitManager
...
+ (void)recieveMicRequest:(NSString *)uid name:(NSString *)name block:(popupManagerCallback)block {
    if (name && name.length && uid && uid.length) {
        ...
        alertCancelModel.title = @"取消";
        alertCancelModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantMicPermission:uid result:NO];
            block(NO);
        };
        ...
        alertModel.title = @"同意";
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantMicPermission:uid result:YES];
            block(NO);
        };
...
+ (void)recieveCameraRequest:(NSString *)uid name:(NSString *)name block:(popupManagerCallback)block {
    if (name && name.length && uid && uid.length) {
        ...
        alertCancelModel.title = @"取消";
        alertCancelModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantCamPermission:uid result:NO];
            if (block) {
                block(NO);
            }
        };
        ...
        alertModel.title = @"同意";
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantCamPermission:uid result:YES];
            if (block) {
                block(YES);
            }
        };
...
+ (void)recieveShareStatus:(BOOL)isShare name:(nullable NSString *)name type:(int)type {
    if (isShare) {
        NSString *msg = [NSString stringWithFormat:@"%@正在共享%@", name, (EduBigClassShareType)type == kShareTypeScreen ? @"屏幕" : @"白板"];
...
+ (void)recieveShareRequest:(NSString *)uid name:(NSString *)name block:(popupManagerCallback)block {
    if (name && name.length && uid && uid.length) {
        ...
        alertCancelModel.title = @"取消";
        alertCancelModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantSharePermission:uid result:NO];
            block(NO);
        };
        ...
        alertModel.title = @"同意";
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager grantSharePermission:uid result:YES];
            block(NO);
        };
...
+ (void)startRecordRequest {
    //[EduBigClassRTMManager requestForRecord];
...
+ (void)recieveRecordRequest:(NSString*)uid name:(NSString*)name block:(nullable popupManagerCallback)block {
    if (name && name.length && uid && uid.length) {
        ...
        alertCancelModel.title = @"取消";
        alertCancelModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
            //[EduBigClassRTMManager acceptRecordRequest:uid result:NO];
            if (block) {
                block(NO);
            }
        };
        ...
        alertModel.title = @"同意";
        alertModel.alertModelClickBlock = ^(UIAlertAction * _Nonnull action) {
           // [EduBigClassRTMManager acceptRecordRequest:uid result:YES];
            if (block) {
                block(YES);
            }
        };
...
+ (void)forceTurnOnOffMicOfUser:(BOOL)on uid:(NSString *)uid {
   // [EduBigClassRTMManager forceTurnOnOffMicOfUser:uid status:on block:^(BOOL result, RTMACKModel * _Nonnull model) {
    //}];
}

+ (void)forceTurnOnOffCamOfUser:(BOOL)on uid:(NSString *)uid {
    //[EduBigClassRTMManager forceTurnOnOffCamOfUser:uid status:on block:^(BOOL result, RTMACKModel * _Nonnull model) {
    //}];
}
...
+ (void)forceTurnOnOffShareOfUser:(BOOL)on uid:(NSString *)uid {
    if (!on) {
        ...
        [[AlertActionManager shareAlertActionManager] showWithMessage:@"是否打开共享权限" actions:@[alertCancelModel, alertModel]];
    } else {
        AlertActionModel *alertCancelModel = [[AlertActionModel alloc] init];
        alertCancelModel.title = @"确认";
        __weak typeof(self) weakSelf = self;
