...
- (void)joinHostRoomWithToken:(NSString *)token roomID:(NSString *)roomID uid:(NSString *)uid {
    ...
    [self.hostRoom joinRoom:token userInfo:userInfo roomConfig:config];
...
- (void)joinGroupRoomWithToken:(NSString *)token roomID:(NSString *)roomID uid:(NSString *)uid {
    ...
    config.isAutoSubscribeAudio = YES;
    config.isAutoSubscribeVideo = YES;
    [self.groupRoom joinRoom:token userInfo:userInfo roomConfig:config];
...
- (void)setHostRoomRemoteVideo:(ByteRTCVideoCanvas *)videoCanvas
    ...
    if (videoCanvas.view) {
        ...
        streamKey.roomId = self.hostRoom.getRoomId;
        streamKey.streamIndex = ByteRTCStreamIndexMain;
        
        [self.rtcKit setRemoteVideoCanvas:streamKey withCanvas:videoCanvas];
    }
}
...
- (void)setGroupRoomRemoteVideo:(ByteRTCVideoCanvas *)videoCanvas
    ...
    if (videoCanvas.view) {
        ByteRTCRemoteStreamKey *streamKey = [[ByteRTCRemoteStreamKey alloc] init];
        streamKey.userId = uid;
        streamKey.roomId = self.groupRoom.getRoomId;
        streamKey.streamIndex = ByteRTCStreamIndexMain;
