...
@interface EduSmallClassRTCManager () <ByteRTCVideoDelegate>

@property (nonatomic, strong) EduSCRoomParamInfoModel *paramInfoModel;
...
@implementation EduSmallClassRTCManager

+ (EduSmallClassRTCManager *_Nullable)shareRtc {
    static EduSmallClassRTCManager *eduSmallClassRTCManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        eduSmallClassRTCManager = [[EduSmallClassRTCManager alloc] init];
    });
    return eduSmallClassRTCManager;
}
...
- (void)joinChannelWithSession:(EduSCRoomVideoSession *)videoSession token:(NSString *)token {
...
- (void)updateRtcVideoParams {
    ...
    localSolution.width = [EduSCSettingsService getResolution].width;
    localSolution.height = [EduSCSettingsService getResolution].height;

    localSolution.frameRate = [EduSCSettingsService getFrameRate];
    localSolution.maxBitrate = [EduSCSettingsService getKBitRate];
...
- (void)rtcRoom:(ByteRTCRoom *)rtcRoom onLocalStreamStats:(ByteRTCLocalStreamStats *)stats {
    ...
    [self updateEduSCRoomParamInfoModel];
}
...
- (void)rtcEngine:(ByteRTCVideo * _Nonnull)engine onLocalAudioPropertiesReport:(NSArray<ByteRTCLocalAudioPropertiesInfo *> * _Nonnull)audioPropertiesInfos {
    ...
    for (int i = 0; i < audioPropertiesInfos.count; i++) {
        ...
        if (model.audioPropertiesInfo.linearVolume > minVolume) {
            ...
            self.localVideoSession.volume = model.audioPropertiesInfo.linearVolume;
        }else {
            self.localVideoSession.volume = 0;
        }
    }
    if ([self.delegate respondsToSelector:@selector(rtcManager:reportAllAudioVolume:)]) {
        [self.delegate rtcManager:self reportAllAudioVolume:[parDic copy]];
...
- (void)startScreenSharingWithParam:(ByteRTCScreenCaptureParam *_Nonnull)param mediaType:(ByteRTCScreenMediaType)mediaType preferredExtension:(NSString *)extension groupId:(NSString *)groupId {
    ...
    CGSize videoSize = [EduSCSettingsService getScreenResolution];
    screenSolution.width = videoSize.width;
    screenSolution.height = videoSize.height;
    screenSolution.frameRate = [EduSCSettingsService getScreenFrameRate];
    screenSolution.maxBitrate = [EduSCSettingsService getScreenKBitRate];
...
- (void)updateEduSCRoomParamInfoModel {
    if ([self.delegate respondsToSelector:@selector(eduSmallClassRTCManager:changeParamInfo:)]) {
        [self.delegate eduSmallClassRTCManager:self changeParamInfo:self.paramInfoModel];
    }
}

#pragma mark - getter
- (EduSCRoomParamInfoModel *)paramInfoModel {
    if (!_paramInfoModel) {
        _paramInfoModel = [[EduSCRoomParamInfoModel alloc] init];
    }
