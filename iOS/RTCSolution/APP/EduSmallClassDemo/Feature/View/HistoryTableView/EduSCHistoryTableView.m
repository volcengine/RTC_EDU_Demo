...
@interface EduSCHistoryTableView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation EduSCHistoryTableView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    EduSCHistoryTableCell *cell = [tableView dequeueReusableCellWithIdentifier:@"EduSCHistoryTableCellID" forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    EduSmallClassControlRecordModel *mdel = self.dataLists[indexPath.row];
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    EduSmallClassControlRecordModel *model = self.dataLists[indexPath.row];
...
- (void)deleteVideoRecord:(EduSmallClassControlRecordModel *)model tableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath{
    if (IsEmptyStr(model.vid)) {
        return;
    }
    __weak __typeof(self) wself = self;
    [EduSmallClassRTMManager deleteVideoRecord:model.vid block:^(RTMACKModel * _Nonnull ackModel) {
        if (ackModel.result) {
            NSMutableArray *lists = [wself.dataLists mutableCopy];
            EduSmallClassControlRecordModel *deleteModel = nil;
            for (EduSmallClassControlRecordModel *cModel in lists) {
...
- (UITableView *)settingsTableView {
    if (!_settingsTableView) {
        ...
        [_settingsTableView registerClass:EduSCHistoryTableCell.class forCellReuseIdentifier:@"EduSCHistoryTableCellID"];
