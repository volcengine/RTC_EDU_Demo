...
@property (nonatomic, strong) EduSCRoomBottomViewModel *viewModel;
@end

@implementation EduSCRoomBottomView
...
- (instancetype)initWithViewModel:(EduSCRoomBottomViewModel *)viewModel {
    ...
    if (self) {
        ...
        BOOL isRecording = self.viewModel.roomModel.record_status == 1;
        [self updateButton:RoomBottomItemRecord status:isRecording];
    }
    return self;
}

- (void)buttonAction:(EduSCRoomItemButton *)itemButton {
...
- (void)addSubviewAndConstraints {
    ...
    for (int i = 0; i < groupNum * 2; i++) {
        EduSCRoomItemButton *button = [[EduSCRoomItemButton alloc] init];
...
- (void)updateButton:(RoomBottomItem)item status:(BOOL)isActive {
    EduSCRoomItemButton *selectButton = [self getButtonOfItem:item];
    if (selectButton) {
...
- (EduSCRoomItemButton *)getButtonOfItem:(RoomBottomItem)item {
    for (EduSCRoomItemButton *button in self.buttonLists) {
...
- (ButtonStatus)getStatusOfItem:(RoomBottomItem)item {
    ...
    EduSCRoomItemButton *selectButton = nil;
    for (EduSCRoomItemButton *button in self.buttonLists) {
...
- (void)updateUserCount:(NSInteger)count {
    EduSCRoomItemButton *button = [self getButtonOfItem:RoomBottomItemUserList];
    button.tagNum = count;
}
