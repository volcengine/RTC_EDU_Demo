...
@interface EduSCRoomNavView ()
@property (nonatomic, strong) EduSCNavViewModel *viewModel;
@property (nonatomic, strong) EduSCRoomItemButton *switchCameraBtn;
@property (nonatomic, strong) EduSCRoomItemButton *switchSpeakerBtn;
@property (nonatomic, strong) EduSCRoomItemButton *hangeupButton;
...
@implementation EduSCRoomNavView
...
- (void)hangeupButtonAction:(EduSCRoomItemButton *)sender {
    if ([self.delegate respondsToSelector:@selector(EduSCRoomNavView:itemButton:didSelectStatus:)]) {
        [self.delegate EduSCRoomNavView:self itemButton:sender didSelectStatus:RoomNavItemHangeup];
    }
}

- (void)switchCameraBtn:(EduSCRoomItemButton *)sender {
    if ([self.delegate respondsToSelector:@selector(EduSCRoomNavView:itemButton:didSelectStatus:)]) {
        [self.delegate EduSCRoomNavView:self itemButton:sender didSelectStatus:RoomNavItemSwitchCamera];
    }
}

- (void)switchSpeakerBtn:(EduSCRoomItemButton *)sender {
    if ([self.delegate respondsToSelector:@selector(EduSCRoomNavView:itemButton:didSelectStatus:)]) {
        [self.delegate EduSCRoomNavView:self itemButton:sender didSelectStatus:RoomNavItemSwitchSpeaker];
    }
}

#pragma mark - Publish Action

- (void)setEduSmallClassTime:(NSInteger)EduSmallClassTime {
    self.secondValue = EduSmallClassTime;
    [self.timer resume];
}
...
- (void)timerMethod {
    NSInteger restTime = MaxEduSmallClassLimitedTimeInSecs - self.secondValue;
...
- (BaseButton *)switchCameraBtn {
    if (!_switchCameraBtn) {
        _switchCameraBtn = [[EduSCRoomItemButton alloc] init];
...
- (BaseButton *)switchSpeakerBtn {
    if (!_switchSpeakerBtn) {
        _switchSpeakerBtn = [[EduSCRoomItemButton alloc] init];
...
- (BaseButton *)hangeupButton {
    if (!_hangeupButton) {
        _hangeupButton = [[EduSCRoomItemButton alloc] init];
