//
//  EduSCMoreSettingPlanViewContainer.m
//  EduSmallClassDemo
//
...
@interface EduSCColorButton : UIButton
...
@property (nonatomic, strong) EduSCSilder * widthSlider;
...
@implementation EduSCMoreSettingPlanView
...
- (void)_createUIComponents {
    ...
    self.widthSlider = [EduSCSilder new];
    WeakSelf
    ...
    for (NSString * colorHex in SelectColorItemsHex()) {
        EduSCColorButton * button = [EduSCColorButton buttonWithType:UIButtonTypeCustom];
        ...
        [button addActionBlock:^(UIButton *btn) {
            ...
            for (int i = 0; i < wself.colorBtnArray.count; i++) {
                EduSCColorButton *tmpBtn = wself.colorBtnArray[i];
                if (tmpBtn != btn && [tmpBtn isKindOfClass:[EduSCColorButton class]]) {
...
@implementation EduSCColorButton
...
@interface EduSCToolView ()
@property(strong, nonatomic) NSMutableArray<UIButton *> *buttons;
@property(strong, nonatomic) NSMutableArray<EduSCMoreSettingPlanView *> *settingViews;
@property(strong, nonatomic) UIStackView *stackView;

@property(assign, nonatomic) BOOL isHiddenButton;
@end

@implementation EduSCToolView : UIView
...
- (instancetype)initWithWhiteBoard:(ByteWhiteBoard *)wb delegate:(id<EduSCMoreSettingPlanViewContainerDelegate>)delegate {
    ...
    if (self) {
        ...
        for (int i = 1; i < 4; ++i) {
            EduSCMoreSettingPlanView *settingView;
            if (i == 1) {
                settingView = [[EduSCMoreSettingPlanView alloc] initWithDelegate:self.delegate type:WBDrawTypePen];
                settingView.delegate = self.delegate;
                NSDictionary *dic = @{@"type" : @(WBDrawTypePen) };
                [settingView setDefualtSetting:dic];
            } else if (i == 2) {
                settingView = [[EduSCMoreSettingPlanView alloc] initWithDelegate:self.delegate type:WBDrawTypeTxt];
                settingView.delegate = self.delegate;
                NSDictionary *dic = @{@"type" : @(WBDrawTypeTxt) };
                [settingView setDefualtSetting:dic];
            } else if (i == 3) {
                settingView = [[EduSCMoreSettingPlanView alloc] initWithDelegate:self.delegate type:WBDrawTypeShape];
                settingView.delegate = self.delegate;
...
-(void)_clickToolBarBtn:(UIButton *)sender {
    ...
    switch (index) {
        ...
        case 3:
            ...
            if (sender.selected) {
                EduSCMoreSettingPlanView *plan = [self.settingViews lastObject];
