...
@property (nonatomic, strong) RoomBottomViewModel *viewModel;
@end
...
- (void)buttonAction:(RoomItemButton *)itemButton {
...
- (void)updateUserCount:(NSInteger)count {
    RoomItemButton *button = [self getButtonOfItem:RoomBottomItemUserList];
    button.tagNum = count;
}
...
- (void)updateButton:(RoomBottomItem)item status:(BOOL)isActive {
    RoomItemButton *selectButton = [self getButtonOfItem:item];
    if (selectButton) {
...
- (RoomItemButton *)getButtonOfItem:(RoomBottomItem)item {
