...
@interface RoomWhiteBoardView() <WBMoreSettingPlanViewContainerDelegate,UITableViewDelegate,UITableViewDataSource>

@property (nonatomic, strong) WBToolView *toolView;
...
@property (nonatomic, strong) WhiteBoardViewModel *viewModel;
...
@implementation RoomWhiteBoardView
...
- (instancetype)initWithViewModel:(WhiteBoardViewModel *)viewModel {
    ...
    if (self) {
        ...
        [self.KVOController observe:self.viewModel.localVideoSession keyPath:@keypath(self.viewModel.localVideoSession, hasSharePermission) options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
            ...
            NSLog(@"Meeting hasSharePermission changed = %d",hasSharePermission);
        }];
...
//            NSLog(@"meeting volume changed = %ld",(long)volume);
...
- (WBToolView *)toolView {
    if (!_toolView) {
        _toolView = [[WBToolView alloc] initWithWhiteBoard:self.board delegate:self];
    }
    return _toolView;
}
