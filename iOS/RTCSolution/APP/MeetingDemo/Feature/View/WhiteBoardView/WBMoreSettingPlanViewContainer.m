//
//  WBMoreSettingPlanViewContainer.m
//  MeetingDemo
//
//  Created by ByteDance on 2022/11/2.
//
#import "BaseButton.h"
#import "WBMoreSettingPlanViewContainer.h"
#import <Masonry.h>
...
@interface WBColorButton : UIButton
...
@property (nonatomic, strong) WBSilder * widthSlider;
...
@implementation MoreSettingPlanView
...
- (void)_createUIComponents {
    ...
    for (NSString * colorHex in SelectColorItemsHex()) {
        WBColorButton * button = [WBColorButton buttonWithType:UIButtonTypeCustom];
        ...
        [button addActionBlock:^(UIButton *btn) {
            ...
            for (int i = 0; i < wself.colorBtnArray.count; i++) {
                WBColorButton *tmpBtn = wself.colorBtnArray[i];
                if (tmpBtn != btn && [tmpBtn isKindOfClass:[WBColorButton class]]) {
...
@interface WBToolView ()
@property(strong, nonatomic) NSMutableArray<UIButton *> *buttons;
@property(strong, nonatomic) NSMutableArray<MoreSettingPlanView *> *settingViews;
@property(strong, nonatomic) UIStackView *stackView;

@property(assign, nonatomic) BOOL isHiddenButton;
@end

@implementation WBToolView : UIView
...
- (instancetype)initWithWhiteBoard:(ByteWhiteBoard *)wb delegate:(id<WBMoreSettingPlanViewContainerDelegate>)delegate {
    ...
    if (self) {
        ...
        for (int i = 1; i < 4; ++i) {
            MoreSettingPlanView *settingView;
            if (i == 1) {
                settingView = [[MoreSettingPlanView alloc] initWithDelegate:self.delegate type:WBDrawTypePen];
                settingView.delegate = self.delegate;
                NSDictionary *dic = @{@"type" : @(WBDrawTypePen) };
                [settingView setDefualtSetting:dic];
            } else if (i == 2) {
                settingView = [[MoreSettingPlanView alloc] initWithDelegate:self.delegate type:WBDrawTypeTxt];
                settingView.delegate = self.delegate;
                NSDictionary *dic = @{@"type" : @(WBDrawTypeTxt) };
                [settingView setDefualtSetting:dic];
            } else if (i == 3) {
                settingView = [[MoreSettingPlanView alloc] initWithDelegate:self.delegate type:WBDrawTypeShape];
                settingView.delegate = self.delegate;
...
-(void)_clickToolBarBtn:(UIButton *)sender {
    ...
    switch (index) {
        ...
        case 3:
            ...
            if (sender.selected) {
                MoreSettingPlanView *plan = [self.settingViews lastObject];
