...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.isMore) {
        if ([model.title isEqualToString:@"用户名"]) {
        ...
        }else if ([model.title isEqualToString:@"资质证照"]) {
            LicenseViewController *next = [[LicenseViewController alloc] init];
            [self.navigationController pushViewController:next animated:YES];
        }
            ...
            [self jumpToWeb:@"https://www.volcengine.com/docs/6348/70241"];
...
- (NSArray *)dataLists {
    if (!_dataLists) {
        ...
        MenuCellModel *model21 = [[MenuCellModel alloc] init];
        model21.title = @"资质证照";
        model21.isMore = YES;
        [lists addObject:model21];
...
- (BaseButton *)logoutButton {
    if (!_logoutButton) {
        ...
        [_logoutButton setTitleColor:[ThemeManager titleLabelTextColor] forState:UIControlStateNormal];
    }
...
- (BaseButton *)deletAccountButton {
    if (!_deletAccountButton) {
        ...
        [_deletAccountButton setTitleColor:[ThemeManager titleLabelTextColor] forState:UIControlStateNormal];
    }
